...
var FileViewerInterface = function(options){
  ...
  if(!options.getvariables)options.getvariables=c4iconfigjs.getvariables;
  if(!options.adagucservice)options.adagucservice=c4iconfigjs.adagucservice;
  if(!options.xml2jsonservice)options.xml2jsonservice=c4iconfigjs.xml2jsonservice;  
...
//      
  ...
  function getVariableInfoFromServerViaOpenDAP(){
    ...
    xml2jsonrequestURL = options.xml2jsonservice+'';
    ...
    var url = options.getvariables+"service=getvariables&request="+URLEncode(options.prettyquery);
    $.ajax({
      url: url,
          crossDomain:true,
          dataType:"jsonp"
